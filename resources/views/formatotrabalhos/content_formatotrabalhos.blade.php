...
@section('titulo', 'Formato de Trabalho ')

@section('table-delete', 'formatotrabalhos')
...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            ...
            <h1 id="page-title" class="h3 mb-0 text-gray-800 font-weight-bold">{{ __('Cadastro de Formato de Trabalho') }}</h1>
        </div>

        <!-- Content Datatable -->
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">{{ __('Formato de Trabalho') }}</h6>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table id="datatableFormatoTrabalhos" class="datatable table table-sm table-responsive text-center rounded"
                        ...
                        <tbody>
                            @foreach ($formatotrabalhos as $obj)
                                <tr>
                                    <th class="align-middle border-left">{{ $obj->id }}</th>
                                    <td class="align-middle border-left">{{ $obj->fdt_nome }}</td>
                                    <td class="align-middle th-sm border-left border-right">
                                        ...
                                        <a href="#" class="btn_crud btn btn-danger btn-sm" data-toggle="tooltip"
                                            onclick="return confirmDeletion({{ $obj->id }}, '{{ $obj->fdt_nome }}', '{{ strtolower(class_basename($obj)) }}')"
                                            title="Excluir"><i class="fas fa-trash-alt"></i></a>
                                    </td>
                                </tr>
                            @endforeach
        ...
        <div class="modal-dialog modal-dialog-scrollable" role="document">
            <div class="modal-content">
                <div class="modal-header bg-success">
                    <h5 class="modal-title text-white font-weight-bold" id="addModalLabel">{{ __('Novo Formato de Trabalho') }}</h5>
        ...
        <div class="modal-dialog modal-dialog-scrollable" role="document">
            <div class="modal-content">
                <div class="modal-header bg-warning">
                    <h5 class="modal-title text-dark font-weight-bold" id="editModalTitle">{{ __('Alterar Formato de Trabalho') }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="/formatotrabalhos" method="POST" id="editForm">
                        {{ csrf_field() }}
                        {{ method_field('PUT') }}
                        <div class="form-group">
                            <label class="mb-0" for="fdt_nome">Nome</label>
        ...
        <div class="modal-dialog modal-dialog-scrollable" role="document">
            <div class="modal-content">
                <div class="modal-header bg-info">
                    <h5 class="modal-title text-white font-weight-bold" id="viewModalTitle">{{ __('Ver Formato de Trabalho') }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="deleteModalTitle"
        ...
        <div class="modal-dialog modal-dialog-scrollable" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="/formatotrabalhos" method="POST" id="deleteForm">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
...
@section('script_pages')
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            var table = $('#datatableFormatoTrabalhos').DataTable();
            ...
            table.on('click', '.edit', function() {
                ...
                $('#editModal #fdt_nome').val(data[1]);

                $('#editForm').attr('action', '/formatotrabalhos/' + data[0]);
            ...
            table.on('click', '.delete', function() {
                ...
                $('#deleteForm').attr('action', '/formatotrabalhos/' + data[0]);
